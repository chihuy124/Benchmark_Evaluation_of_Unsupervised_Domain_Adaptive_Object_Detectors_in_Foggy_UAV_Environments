...
from tools.NMS import nms_per_category
...
def do_coco_evaluation(
    ...
    if "bbox" in iou_types:
        ...
        logger.info("Sap vao ham prepare_for_coco_detection")
...
def prepare_for_coco_detection(predictions, dataset):
    ...
    for image_id, items in image_dict.items():
        ...
        filtered_boxes, filtered_scores, keep = nms_per_category(boxes, scores, categories, iou_threshold=0.4)
...
def ordered_print(results_per_category):
    logger = logging.getLogger("fcos_core.trainer")
    print(results_per_category)
    # for cls_name in range(20):
    #     logger.info(str(cls_name) + ': ' + results_per_category[cls_name])
    #     AP = float(results_per_category[cls_name]) * 100
        # AP = round(AP, 1)
    # for cls_name in cls_names:
    #     logger.info(cls_name+': '+ results_per_category[cls_name])
    #     AP = float(results_per_category[cls_name])*100
    #     AP = round(AP,1)
    #     copypaste.append(str(AP))
    #
    # logger.info('&'.join(copypaste))
# def ordered_print(results_per_category):
#     cls_names = ['person', 'rider', 'car', 'truck', 'bus', 'train', 'motorcycle', 'bicycle']
#     copypaste = []
#
#     logger = logging.getLogger("fcos_core.trainer")
#     for cls_name in cls_names:
...
def evaluate_predictions_on_coco(
    ...
    results_per_category_ap50_95 = {}
    results_per_category_ap75 = {}

    if len(coco_gt.getCatIds())> 3:
        ...
        # max dets index -1: typically 100 per image
            ...
            if precision.size:
                ...
                ap = float('nan')
            results_per_category[str(nm["name"])] = f'{float(ap):0.3f}'
            
            # --- AP50:95 (mAP) ---
            precision_ap50_95 = precisions[:, :, idx, 0, -1]  
            valid_precision_ap50_95 = precision_ap50_95[precision_ap50_95 > -1]  
            if valid_precision_ap50_95.size:
                ap50_95 = np.mean(valid_precision_ap50_95)  
            else:
                ap50_95 = float('nan')
            results_per_category_ap50_95[str(nm["name"])] = f'{float(ap50_95):0.3f}'
            
            # --- AP75 ---
            precision_ap75 = precisions[5, :, idx, 0, -1]  
            valid_precision_ap75 = precision_ap75[precision_ap75 > -1] 
            if valid_precision_ap75.size:
                ap75 = np.mean(valid_precision_ap75)  
            else:
                ap75 = float('nan')

            results_per_category_ap75[str(nm["name"])] = f'{float(ap75):0.3f}'  
        
        print("\nAP50:95 per category:")
        ordered_print(results_per_category_ap50_95)
        
        print("\nAP50 per category:")
        ordered_print(results_per_category)

        print("\nAP75 per category:")
        ordered_print(results_per_category_ap75)
