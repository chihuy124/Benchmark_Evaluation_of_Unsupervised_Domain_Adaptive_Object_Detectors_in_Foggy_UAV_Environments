...
def parse_model(d, ch, verbose=True, warehouse_manager=None):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d["backbone"] + d["head"]):  # from, number, module, args
        ...
        if m in {
            ...
            C2f_FMB, LDConv, C2f_gConv, C2f_WDBB, C2f_DeepDBB,C2f_DCMB
        }:
            if args[0] == 'head_channel':
                args[0] = d[args[0]]
            c1, c2 = ch[f], args[0]
            ...
            if m in {
                     ...
                     C2f_FMB, C2f_gConv, C2f_WDBB, C2f_DeepDBB,C2f_DCMB
                     }:
        ...
        if isinstance(c2, list) and m not in {ChannelTransformer, PyramidContextExtraction,MutilScaleEdgeInfoGenetator}:
            is_backbone = True
            m_ = m
            m_.backbone = True
        else:
