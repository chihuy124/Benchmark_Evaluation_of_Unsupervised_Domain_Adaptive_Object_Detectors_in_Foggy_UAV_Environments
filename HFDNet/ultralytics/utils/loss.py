...
class v8DetectionLoss:
    ...
    def __call__(self, preds, batch):
        ...
        return loss.sum() * batch_size, loss.detach() # 第二个值为标量，不会梯度计算和反向传播

    def compute_loss(self, preds, batch):
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        loss = torch.zeros(3, device=self.device)  # box, cls, dfl
        # loss = torch.zeros(5, device=self.device)  # box, cls, dfl,gram,mse
        ...
        # targets
        # 要求都在 cuda上计算 
        targets = torch.cat((batch['batch_idx'].view(-1, 1), batch['cls'].view(-1, 1), batch['bboxes']), 1) #[203],[203,1],[203,4] ->[203,1],[203,1],[203,4] = [203,6]
