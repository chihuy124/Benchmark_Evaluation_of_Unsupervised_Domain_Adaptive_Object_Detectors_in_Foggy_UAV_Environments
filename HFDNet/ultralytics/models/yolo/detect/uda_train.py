...
class UDADetectionTrainer(UDABaseTrainer):
    ...
    def uda_get_dataloader(self, dataset_path_S,dataset_path_T, batch_size=16, rank=0, mode_S="train", mode_T="target"):
        """Construct and return dataloader."""
        assert mode_S ,mode_T in ["train","target","val"]
        ...
        shuffle = mode_S == "train"
        if getattr(dataset_S, "rect", False) and shuffle:
            LOGGER.warning("WARNING ⚠️ 'rect=True' is incompatible with DataLoader shuffle, setting shuffle=False")
            shuffle = False
        if getattr(dataset_T, "rect", False) and shuffle:
            LOGGER.warning("WARNING ⚠️ 'rect=True' is incompatible with DataLoader shuffle, setting shuffle=False")
            shuffle = False
        workers = self.args.workers if mode_S == "train" else self.args.workers * 2

        return uda_build_dataloader(dataset_S, dataset_T, batch_size, workers, shuffle, rank)  # return dataloader
    ...
    def get_validator(self):
        ...
        # self.loss_names = "box_loss", "cls_loss", "dfl_loss","daca_bl", "daca_cl", "daca_dl","gram_loss","mmd_loss","mse_loss"  # 伪标签 daca + 域差异
        ...
        # self.loss_names = "box_loss", "cls_loss", "dfl_loss","swd_loss","gmmd_loss","mse_loss"  # 域差异
        # self.loss_names = "box_loss", "cls_loss", "dfl_loss","dss_loss","gmmd_loss","mse_loss"  # 域差异
        # self.loss_names = "box_loss", "cls_loss", "dfl_loss","daca_bl", "daca_cl", "daca_dl", "dss_loss","gmmd_loss","mse_loss"  # 域差异
        # self.loss_names = "box_loss", "cls_loss", "dfl_loss","smmd_loss","mmmd_loss","hmmd"  # 域差异
       
        self.loss_names = "box_loss", "cls_loss", "dfl_loss","psc_box","psc_cls","psc_dfl","dss_loss","gmmd_loss","mse_loss"  # 伪标签 confmix + 域差异
