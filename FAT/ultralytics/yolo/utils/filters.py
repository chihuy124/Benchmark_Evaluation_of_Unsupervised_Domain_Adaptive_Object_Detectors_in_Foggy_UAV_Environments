...
import numpy as np
from ultralytics.yolo.utils.util_filters import rgb2lum, tanh_range, lerp
from ultralytics.yolo.utils.util_filters import *
...
class UsmFilter(Filter):#Usm_param is in [Defog_range]
  ...
  def process(self, img, param, defog_A, IcA):
    ...
    kernel = kernel.to(img.device).double()
    ...
    cv2.imwrite("./img_defog_all.jpg",im )
...
class DefogFilter(Filter):#Defog_param is in [Defog_range]
  ...
  def filter_param_regressor(self, features):
    return tanh_range(*self.cfg.defog_range)(features)  # 修改去雾的范围
  ...
  def process(self, img, param, defog_A, IcA):
    ...
    # print(IcA.shape)
    tx = 1 - param * IcA
    # tx = 1 - 0.65 * IcA  # (65 70 85)
    # tx = 1 - 0.5*IcA

    # tx_1 = np.tile(tx, [1, 1, 1, 3])
    ...
    # cv2.imwrite("./img_defog.jpg",bgr_img )
    # cv2.imwrite("./img_defogb.jpg",img )

    return  (img - defog_A) / tx_1 + defog_A
...
class ImprovedWhiteBalanceFilter(Filter):
  ...
  def filter_param_regressor(self, features):
    ...
    # features = torch.matmul(features,mask)
