...
def xyxy2xywhn_640(x, w=640, h=640, clip=False, eps=0.0):
    # Convert nx4 boxes from [x1, y1, x2, y2] to [x, y, w, h] normalized where xy1=top-left, xy2=bottom-right
    # if clip:
    #     clip_coords(x, (h - eps, w - eps))  # warning: inplace clip
    # 先将标签从608*608的尺寸转到608*304的，再改为归一化为到608*304的，（那就是先只改y）
    rw = w
    rh = h
    x[:, 1] = x[:, 1] - h/2
    x[:, 3] = x[:, 3] - h/2
...
def load_mosaic_defog(img_list_64, size_all,predict_list, device, img_size, mosaic_border, hyp):  # 例批次为16， 输入为64张图片，64个伪标签，输出为16张图片，16个伪标签
    # img 代表需要强增强的图像， predict_list为需要对应变换的标签，4个标签为一个
    # YOLOv5 4-mosaic loader. Loads 1 image + 3 random images into a 4-image mosaic
    labels4_16 = []
    s = img_size # 640 
    s_div_4 = img_size // 4 # 160
    s_div_2 = img_size // 2 # 320
    s_div_3_4 = s_div_2 + s_div_4 # 480
    img_list = [] 
    batchsize = 16 # 先假设批次为16
    i = 0
    #  for img_i, predict_i in zip(imglist, predict_list):
    # 图片是16 * 4的形式
    predict_index = 0 # (0-63)
    i = 0 # i ~0 1 2 3
    img_index = 0
    img_i_list = [0,1,2,3]
    img_i_list_u = random.sample(img_i_list, 2) 
    for img_i in range(size_all):
        # 初始化变量
        if i == 0:
            labels4, segments4 = [], []
            # img_ori_fog_4 = img_list_64[img_index]
            yc, xc = [int(random.uniform(-x, 2 * s + x)) for x in mosaic_border]  # mosaic center x, y
        if i < 4:
            

            # for img_index, img_i_4 in enumerate(img_list_64):
            #     labels4, segments4 = [], []
            #     i = 0 # i ~0 1 2 3
            #     yc, xc = [int(random.uniform(-x, 2 * s + x)) for x in self.mosaic_border]  # mosaic center x, y
            #     for img_i in img_i_4:
            # Load image
            # img_i = img_i[:,152:456,:]


            # if i == img_i_list_u[0] or i == img_i_list_u[1]:
            #     img_i = img_ori_fog_4[i][:,152:456,:]
            #     img = img_i.permute(1,2,0).cpu().detach().numpy()
            # else:
            #     img = img_i.permute(1,2,0).cpu().detach().numpy()
            #     img = img*255
            img_i = img_list_64[predict_index][:,s_div_4:s_div_3_4,:]
            img = img_i.permute(1,2,0).cpu().detach().numpy()



            # cv2.imwrite('./img_o.jpg', img)
            h = s_div_2
            w = s
            # 先暂时这样，不行后面再加从608返回1024*2048的，或者直接重新读图片
            # img_t_weak = deHaze( img/ 255.0) * 255 # 换成传统去雾方法作为弱增强图像
            # place img in img4
            if i == 0:  # top left
                img4 = np.full((s * 2, s * 2, img.shape[2]), 114, dtype=np.uint8)  # base image with 4 tiles
                x1a, y1a, x2a, y2a = max(xc - w, 0), max(yc - h, 0), xc, yc  # xmin, ymin, xmax, ymax (large image)
            ...
            # cv2.imwrite('./img4_orin.jpg', img4)
            img4[y1a:y2a, x1a:x2a] = img[y1b:y2b, x1b:x2b]  # img4[ymin:ymax, xmin:xmax]
            # cv2.imwrite('./img4.jpg', img4)
            # cv2.imwrite('./img_t_weak4.jpg', img_t_weak4)
            padw = x1a - x1b
            padh = y1a - y1b

            # Labels
            # cv2.imwrite('./img_orin.jpg', img)
            labels_copy = predict_list[predict_index].cpu().detach().numpy().copy() # shape (,)
            labels = np.zeros((len(labels_copy),6))
            segments = [] # 默认是空
            if labels.size:
                
                labels_copy[:,:4] = xyxy2xywhn_640(labels_copy[:,:4], w=s, h=s_div_2)
                # labels_copy[:,:4] = xyxy2xywhn(labels_copy[:,:4], w=608, h=304)
                labels[:, 1:5] = xywhn2xyxy(labels_copy[:,:4], w, h, padw, padh)  # normalized xywh to pixel xyxy format   反归一化5,      52.922,      390.36,      63.312,      400.75],
                # labels[:, 1:] = labels_copy[:,:4]  # 取bbox
                #（cls、xywh、conf）
                labels[:, 0] = labels_copy[:,-1] #取cls
                # labels[:, -1] = labels_copy[:,4] #取conf
                labels[:, -1] = 1 #取conf
                segments = [xyn2xy(x, w, h, padw, padh) for x in segments]
            labels4.append(labels)
            segments4.extend(segments)
        ...
        # img4, labels4, segments4 = copy_paste(img4, labels4, segments4, p=hyp['copy_paste'])
        img4,labels4, segments4 = copy_paste(img4, labels4, segments4, p=hyp.copy_paste)
        ...
        # if self.augment:
        #     # HSV color-space
        #     augment_hsv(img4, hgain=hyp['hsv_h'], sgain=hyp['hsv_s'], vgain=hyp['hsv_v'])
        #     # 增加unbias techer里面的强增强方法
        #     if hyp['unbias_teacher_augment'] :
        #         image_pil = Image.fromarray(img4.astype("uint8"), "RGB")
        #         img4 = np.array(self.strong_augmentation(image_pil))
            # Flip up-down
            # if random.random() < hyp['flipud']:
            #     img = np.flipud(img)
            ...
            #     if nl:
            #         labels[:, 1] = 1 - labels[:, 1]
        # 返回对应batchsize的tensor 图片 ，和对应的伪标签
        img_mosaic_i = img4.copy()
        # img_mosaic_i = img_mosaic_i.transpose((2, 0, 1))[::-1]  # HWC to CHW, BGR to RGB
        img_mosaic_i = img_mosaic_i.transpose((2, 0, 1))
        img_mosaic_i = np.ascontiguousarray(img_mosaic_i)
        img_mosaic_i = torch.from_numpy(img_mosaic_i)
        img_mosaic_i = img_mosaic_i.unsqueeze(0)
        #（cls、xywh、conf）
        labels4 = labels4[:,[1,2,3,4,0,5]]
        # labels4 = np.insert(labels4, 4, 1, axis=1)
        labels4 = torch.tensor(labels4).to(device)
        labels4_16.append(labels4)
        if img_index == 0:
            img_mosaic_4_16 = img_mosaic_i
        else:
            img_mosaic_4_16 = torch.cat((img_mosaic_4_16,img_mosaic_i),dim=0)
        img_index += 1
        i = 0
    img_mosaic_4_16 = img_mosaic_4_16.to(device, non_blocking=True).float() / 255.0     
    return img_mosaic_4_16, labels4_16
