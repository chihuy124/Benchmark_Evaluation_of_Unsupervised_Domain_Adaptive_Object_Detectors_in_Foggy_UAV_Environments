...
def build(image_set, args):
    ...
    # --- UIT-DRONE custom ---
    # ========================
    if args.dataset_file == "uitdrone":
        if image_set == "train":
            print("==== Load UIT Drone train dataset ====")
            return DADataset(
                source_img_folder=paths["uitdrone"]['source_train_img'],
                source_ann_file=paths["uitdrone"]['train_anno'],
                target_img_folder=paths["uitdrone"]['target_train_img'],
                target_ann_file=paths["uitdrone"]['train_anno'],
            ...
            print("==== Load UIT Drone test dataset ====")
            return CocoDetection(
                img_folder=paths["uitdrone"]['target_test_img'],
                ann_file=paths["uitdrone"]['test_anno'],
                transforms=make_coco_transforms(image_set),
                return_masks=args.masks,
            ...
            print("==== Load UIT Drone val dataset ====")
            return CocoDetection(
                img_folder=paths["uitdrone"]['source_test_img'],
                ann_file=paths["uitdrone"]['test_anno'],
                transforms=make_coco_transforms(image_set),
                return_masks=args.masks,
        ...
        elif image_set == "test2":
            print("==== Load UIT Drone test2 dataset ====")
            return CocoDetection(
                img_folder=paths["uitdrone"]['target_test_img'],
                ann_file=paths["uitdrone"]['test_anno'],
                transforms=make_coco_transforms(image_set),
    ...
    # ================================
    # --- Cityscapes / default case --
    # ================================
    if args.dataset_file == "cityscapes":
        if image_set == "train":
            print("==== Load Cityscapes train dataset ====")
            if args.cdod:
                return DADataset(
                    source_img_folder=paths["cityscapes"]['train_img'],
                    source_ann_file=paths["cityscapes"]['train_anno'],
                    target_img_folder=paths["foggy_cityscapes"]['train_img'],
                    target_ann_file=paths["foggy_cityscapes"]['train_anno'],
                    transforms=make_coco_transforms(image_set),
                    return_masks=args.masks,
                    cache_mode=False,
                    local_rank=args.rank,
            ...
            print("==== Load Cityscapes val dataset ====")
            return CocoDetection(
            ...
            print("==== Load Foggy Cityscapes test dataset ====")
            return CocoDetection(
    ...
    # ================================
    # --- Default fallback (COCO) ----
    # ================================
    print("==== Load default COCO dataset ====")
    mode = 'instances'
    coco_default_paths = {
    ...
    if image_set in coco_default_paths:
        img_folder, ann_file = coco_default_paths[image_set]
        return CocoDetection(
    ...
    raise ValueError(f"Unsupported combination: dataset_file={args.dataset_file}, image_set={image_set}")
...
def get_paths(root):
    ...
    return {
        ...
        'uitdrone': {
            'source_train_img': root / 'uit_drone/images/source_train',
            'target_train_img': root / 'uit_drone/images/target_train',
            'source_test_img': root / 'uit_drone/images/source_test',
            'target_test_img': root / 'uit_drone/images/target_test',
            'train_anno': root / 'uit_drone/train.json',
            'test_anno': root / 'uit_drone/test.json',
        },
