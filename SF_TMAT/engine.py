...
def evaluate(model: torch.nn.Module,
    ...
    for i, (images, masks, annotations) in enumerate(data_loader_val):
        ...
        if output_result_labels:
            results = get_pseudo_labels(logits_all[-1], boxes_all[-1], [0.5 for _ in range(4)])
            for anno, res in zip(annotations, results):
                image_id = anno['image_id'].item()
                if image_id not in dataset_annotations:
                    print(f"警告：image_id {image_id} 不在 coco_data['images'] 中！跳过此注释。")
                    continue  # 跳过不存在的 image_id
                if not isinstance(dataset_annotations[image_id], list):
                    print(f"错误：dataset_annotations[{image_id}] 的类型为 {type(dataset_annotations[image_id])}，重置为空列表。")
                    dataset_annotations[image_id] = []  # 重置为列表
                # print(image_id)
                ...
                # 新增score的提取（假设res中存在scores字段）
                scores = res['scores'].detach().cpu().numpy().tolist()
                for label, score, box in zip(res['labels'].detach().cpu().numpy().tolist(),
                                             scores,
                                             converted_boxes):
                    pseudo_anno = {
                        'id': 0,
                        'image_id': image_id,
                        'category_id': label,
                        'score': float(score),  # 显式转换为float类型
                        'iscrowd': 0,
                        'area': box[-2] * box[-1],
                        'bbox': box
                    }
    ...
    if output_result_labels:
        ...
        for image_anno in dataset_annotations.values():  # 如果是列表，改成直接 for image_anno in dataset_annotations
            if not isinstance(image_anno, list):
                print(f"跳过异常值：{image_anno}，类型为：{type(image_anno)}")
                continue
            for box_anno in image_anno:
                box_anno['id'] = id_cnt
                id_cnt += 1
