...
class CocoEval(COCOeval):
    ...
    def evaluate(self):
        ...
        if p.useCats:
            ...
            if not p.catIds:
                p.catIds = self.cocoGt.getCatIds()
            p.catIds = list(np.unique(p.catIds))
    ...
    def summarize_ap(self, if_print=True):
        ...
        def _summarize(iou_thr=None, area_rng='all', max_dets=100):
            ...
            K = s.shape[2]
            aps = np.asarray([np.mean(s[:, :, i, :]) for i in range(K)])
            
            aps_clean = [ap for ap in aps if ap > -0.001]
            #mean_ap = np.mean(aps_clean)
            mean_ap = float(np.mean(aps_clean)) if len(aps_clean) else float('nan')
            ...
            if if_print:
                ...
                for idx, ap in enumerate(aps):
                    if p.useCats:
                        catId = catIds[idx]
                        # dùng cocoGt.cats[catId] hoặc loadCats để an toàn
                        name = self.cocoGt.cats[catId]['name'] if catId in self.cocoGt.cats \
                               else self.cocoGt.loadCats([catId])[0]['name']
                        print(f'\tAP of category [{name}]:\t\t{ap}')
                    else:
                        print(f'\tAP (no category):\t\t{ap}')
            return aps

        if not self.eval:
            raise Exception('Please run accumulate() first')
        # 计算并打印不同 IoU 阈值下的 mAP
        map_50_95 = _summarize(iou_thr=None, max_dets=self.params.maxDets[2])  # IoU=0.5:0.95
        map_50 = _summarize(iou_thr=0.5, max_dets=self.params.maxDets[2])  # IoU=0.5
        map_75 = _summarize(iou_thr=0.75, max_dets=self.params.maxDets[2])  # IoU=0.75
        return {
            "mAP_50_95": map_50_95,
            "mAP_50": map_50,
            "mAP_75": map_75
...
class CocoEvaluator:
    ...
    def reset(self):
        ...
        self.img_ids = []
        self.eval_imgs = []
        self.coco_eval = CocoEval(self.coco_gt)
    ...
    def update(self, predictions):
        ...
        if not self.coco_eval.params.catIds and self.coco_eval.params.useCats:
            self.coco_eval.params.catIds = self.coco_gt.getCatIds()
