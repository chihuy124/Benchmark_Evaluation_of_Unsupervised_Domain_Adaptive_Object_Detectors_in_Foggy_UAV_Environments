...
class SetCriterion(nn.Module):
    ...
    def dynamic_threshold2(self):
        ...
        logits_means = [
            s.item() / n.item() if n.item() > 0 else 0.0
            for s, n in zip(self.logits_sum, self.logits_count)
        ]
    
        logits_variance = [
            math.sqrt(max((s.item() / n.item()) ** 2 - mean ** 2, 0)) if n.item() > 0 else 0.0
            for s, mean, n in zip(self.logits_sum, logits_means, self.logits_count)
        ]
    
        return logits_means, logits_variance
